...
of this microplate reader machines include software to alleviate this problem but
many times shows a lack of standarization. Although several good R packages exists many of them are design to solve specific problems lossing generability. For example some features from @plater were used to build `mpxtractor`.  To clarify, `mpxtractor` can process
...
and layout files over a microplate shape. The latter can be printed to be use in
...
generate the layout file. Using `mpxtractor` we can plot the layout file.  or load 
it into a tidy dataframe.
Now we want to set the microplates and run the experiment in the lab, for this we can use the `plot_layout_file()` function to have a visualization of our experimental design For example, to avoid edge effects due to temperature, evaporation, etc. the wells around the perimeter of the plate might be more sensitive to this effects, visualizing the layout design over the microplate is more easy to check whether the control or the samples are well positioned in the plate. In **Fig.1** is shown the `basic` containing control or sample experiment while `condition` contain the type of sugar to be used in the experiment.
...
```{r}
# Path to files
file1 <- system.file("extdata", "test_gr_spectramax.txt", package = "mpxtractor")
...
```{r, eval = TRUE, fig.width = 10, fig.height = 7 ,out.width = "100%", out.height = "100%"}
...
ggplot2::ggsave(filename = "test_vignette.png", plot = plt, width = 50, height = 30, units = "cm")
```

**Note:** To improve visualization the output of `plot_gr_microplate()` can be save 
into a png file using the command below.
```{r, eval = FALSE}
