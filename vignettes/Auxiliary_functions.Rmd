 ...
 * If the attribute used to calculate growth rates contain Inf/-Inf values this are tranformed to `NAs` and impute using k=1, this means that one value to the right and one to the left are used to calculate the mean of that missing value.
...
Is very important to note that the difference between time points must be equal for all them. Otherwise the time series is not complete. Remeber the example with
fluorstar machines.
