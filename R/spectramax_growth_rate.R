...
format_time <- function(sp_data, time) {
  if (is.null(time)) {
...
background_correction_impute <- function(sp_data_layout) {
  ...
  df_tmp_gr <- dplyr::mutate(sp_data_layout, min_measurement = min(Measurement))
...
calculate_growth_rate <- function(df_tmp_gr, windowsize) {
  df_tmp_gr$Diff_time <- (df_tmp_gr$Time[2,] - df_tmp_gr$Time[1,])
