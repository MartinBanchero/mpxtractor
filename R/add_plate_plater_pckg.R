add_plate_file <- function (data, file, well_ids_column) {
  ...
  to_add <- read_plate_file(file, "wellIds")
  check_well_ids_column_name(well_ids_column)
  validate_column_is_in_data(data, well_ids_column)
  missing_leading_zeroes <- are_leading_zeroes_missing(data,
                                                       well_ids_column,
  ...
  if (!(all(to_add$wellIds %in% data[[well_ids_column]]))) {
    stop(wrong_wells_error_message(data, well_ids_column,
                                   to_add))
  }
  ...
  class(result) <- union(original_class, c("tbl_df", "tbl",
                                           "data.frame"))
  result
}
