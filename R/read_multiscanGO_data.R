...
# Main function
read_read_multiscanGO_data <- function(file)
{
...
#get the raw file
get_raw_file_clean_multiscanGO <- (file){
...
# format_df
format_df <- (clean_file){
  idx <- grepl('Reading', file)#Get indx of readings
  df <- read.table(text = file[!idx])#get all the lines whith numbers
  
  wd = diff(c(which(idx), length(idx) + 1)) - 1#Calculate the number of lines between readings
  
  df <- cbind(Reading = rep(file[idx], wd),df)#Assign reading to corresponding values 
...
final_format_df <- (df3){
  df_tmp <- df3 %>%
    gather(Col, Measurement, -Reading) %>%
    group_by(Reading, Col) %>%
    mutate(rowID = seq_along(Col)) %>% 
    ungroup() %>%
    mutate(name = interaction(Col, rowID, sep = ''), Col = NULL, rowID = NULL ) %>%
    spread(name, Measurement )
    df_tmp2  <- tidyr::gather(df_tmp, key = 'Wells', value = 'Measurement', -Reading)
}

set_well_ids <- (df_tmp2){
  wells <- df_tmp2$Wells
