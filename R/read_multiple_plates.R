...
# List the files present in the given folder and if pattern is given look for files with that pattern
#
get_files <- function(folder, pattern = NULL)
    {
      ...
      lst_files <- list.files(path = folder, pattern = pattern, recursive = TRUE, full.names = TRUE)
      return(lst_files)
    }
